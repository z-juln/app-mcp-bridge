...
状态：macOS 核心、完整设置与实时调试界面、客户端写入闭环和内部名称迁移均已验收
...
- 通用 MCP/HTTP 接入，以及 Cursor、WorkBuddy 的已验证示例。
- 可由兼容 Agent 安装的第一版通用 Skill。
- App 内通用的 Skill 安装教学入口。
...
| 部分 | 推荐选择 | 原因 |
|---|---|---|
| App | Swift 6 原生 App Bundle | 保持稳定权限身份，通过端口提供服务，并显示程序坞、菜单栏和操作反馈 |
...
当前版本作为带程序坞图标和菜单栏状态入口的 App Bundle 常驻，负责：
...
命令行提供 `start`、`stop`、`status`、`permissions`、`token`、`call` 等管理入口。
需要用户处理权限时，App 会提示并可直接前往对应系统设置。
...
UI Bridge 的连接入口保持通用，不在主界面预置或捆绑某个 Agent。Cursor、WorkBuddy
只作为已验证示例；其他支持 MCP 的客户端使用同一地址和工具。

Skill 位于 Agent 一侧，用于指导连接恢复、读取顺序、动作检查和结果验证。MCP 位于
Bridge 一侧，负责提供工具和基础说明。两者不能互相替代：MCP 尚未连接时无法负责
安装 Skill，MCP 已连接也无法可靠判断宿主是否真正加载了 Skill。

当前 App 已把通用 Skill 作为只读资源随包提供。连接页的“安装 Agent Skill”按钮只展示
并复制一段自包含安装提示词，由用户粘贴给当前 Agent。Agent 根据自己的客户端规则
选择安装位置、备份旧版本并检查结果。UI Bridge 不静默修改客户端目录，也不显示
“已安装/未安装”等猜测状态；未来只有客户端主动上报可信版本时，才增加状态展示。
...
用于 Agent 快照的单次截图与用于本机调试界面的实时预览分开管理。实时预览由常驻
App 内唯一的操控会话中心使用 `SCStream` 持续接收画面；设置窗口、菜单栏和操作
提示只订阅该会话，禁止自行轮询截图或启动截图子进程。每个目标窗口只保留最新一帧，
来不及展示的旧帧直接丢弃。实时页关闭后立即停止所有预览流并清空画面，截图不落盘。
...
当前服务在 `plan_check` 中拦截高影响动作，只有用户明确同意后，客户端才能在同一
动作请求中标记已确认。一次性确认令牌和跨客户端绑定属于后续安全增强。
...
| 客户端滥用本地端口 | 当前使用回环监听和令牌；后续补应用白名单与客户端配对 |
| 日志泄露聊天内容和密码 | 默认只记录元数据，敏感属性遮盖，诊断包需用户主动导出 |
| 高影响操作误执行 | 执行前计划检查、明确确认标记与紧急停止 |
