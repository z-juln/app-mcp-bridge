...
- App 内“安装 Agent Skill”教学按钮和 Skill 安装状态检测。
- 自动更新、正式发布渠道和团队管理。
...
当前仓库已提供通用 Skill，但 App 尚未提供安装教学入口。下一步在连接页增加统一的
“安装 Agent Skill”按钮，点击后只展示并复制一段自包含安装提示词，由用户粘贴给当前
Agent。Agent 根据自己的客户端规则选择安装位置、备份旧版本并自检。UI Bridge 不静默
修改客户端目录，也不显示“已安装/未安装”等猜测状态；未来只有客户端主动上报可信
版本时，才增加状态展示。
