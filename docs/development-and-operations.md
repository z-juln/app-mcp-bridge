...
当前通用 Skill 位于 `skills/macos-ui-control/`，尚未复制进 App，也没有 App 内安装按钮。
源码开发阶段可把下面这段提示词连同仓库路径交给当前 Agent，由 Agent 按自己客户端的
Skill 规范安装：

```text
请从当前 UI Bridge 仓库的 skills/macos-ui-control/ 安装 Agent Skill。
根据你当前客户端支持的方式选择安装位置，不要覆盖其他 Skill；如果已有同名旧版本，
先备份再更新。安装后重新读取 SKILL.md，运行其中的只读自检，并告诉我安装位置和结果。
不要把 UI Bridge 作为子进程启动；连接失败时，只能独立打开 /Applications/UI Bridge.app，
等待健康检查通过后重新连接 MCP。
```

未来的 App 教学按钮会复制同等含义的自包含提示词，并改为引用 App 内随包提供的稳定
只读 Skill 源。它不会绑定指定 Agent，不会通过 MCP 自动安装，也不会扫描或猜测 Skill
是否已经加载。客户端若未明确支持 Skill，仍可只使用 MCP；关键安全规则由 Bridge 强制。
